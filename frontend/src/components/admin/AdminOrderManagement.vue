      ...
      <table class="admin-table">
        ...
        <tbody>
          <tr v-for="order in filteredOrders" :key="order.id" class="order-row">
            ...
            <td>
              <div class="actions">
                <button @click="viewOrderDetails(order)" class="btn-icon">👁️</button>
                <button @click="openEditStatus(order)" class="btn-icon">⚙️</button>
              </div>
            </td>
          </tr>
        </tbody>
...
const openEditStatus = async (order) => {
  const newStatus = prompt("Enter Status (PENDING, PROCESSING, SHIPPED, COMPLETED, CANCELLED):", order.status);
  if (newStatus && newStatus.toUpperCase() !== order.status) {
    try {
...
.search-input:focus { outline: none; border-color: #0990ff; box-shadow: 0 0 0 3px rgba(9, 144, 255, 0.1); }
...
.loader { width: 30px; height: 30px; border: 3px solid #f3f3f3; border-top: 3px solid #0990ff; border-radius: 50%; animation: spin 1s linear infinite; margin-bottom: 10px; }
...
.order-id { font-family: 'Monaco', monospace; font-weight: 700; color: #0990ff; background: #f0f7ff; padding: 4px 8px; border-radius: 6px; }
...
.btn-icon:hover { border-color: #0990ff; background: #f0f9ff; transform: translateY(-1px); }
...
.highlight { color: #0990ff; font-weight: 800; font-size: 1.2rem; }
